...
export const userClaimLead = async (lead) => {
  try {
    const { data } = await axios.patch(
      `http://localhost:4000/api/users/claim/${lead._id}`,
      lead,
      config()
    )
    return data
...
export const userReturnLead = async (lead) => {
  try {
    const { data } = await axios.patch(
      `http://localhost:4000/api/users/return/${lead._id}`,
      lead,
      config()
    )
    return data
